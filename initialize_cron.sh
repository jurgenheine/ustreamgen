...
if [ "$SINGLELIST" == "true" ]
then
    printf "#! /bin/bash\ncd /m3u2strm && python3 main.py \"$SINGLELISTURL\" \"all\" \"$USEGROUP\" \"$MOVIES\" \"$TVSHOWS\" \"$EVENTS\" \"/movies/\" \"/tv/\" \"/events/\"\n" > /root/parse_singlelist.sh
    ...
    if [ "$TVSHOWS" == "true" ]
    then 
        printf "#! /bin/bash\ncd /m3u2strm && python3 main.py \"$TVSHOWURL\" \"tvshows\" \"$USEGROUP\" \"$APOLLO\" \"/tv/\"\n" > /root/parse_tvlist.sh
    ...
    if [ "$EVENTS" == "true" ]
    then
        printf "#! /bin/bash\ncd /m3u2strm && python3 main.py \"$EVENTURL\" \"events\" \"$USEGROUP\" \"$APOLLO\" \"/events/\"\n" > /root/parse_eventlist.sh
